...
- **前置数据检查**：按类目分组需要 offer 的 category 字段。先查 `docs/FINDINGS.md` 与 `test/fixtures/` 确认 eligible/enrolled 响应里是否有类目字段并已被 `buildOfferIndex`/`buildAddedIndex` 保留；若数据不存在，「按类目」选项**不做**（不许做假分组），在 PR 里说明。
...
- **三种特殊行**（13a 底部）：
  ...
  - 无法自动追踪：折叠低透明度行（如 Uber Cash 在 App 内发放）；
  - 未归类返现：折叠低透明度行（`$X · N 笔 对不上已知 benefit`）。
  - **前置数据检查**：后两种行依赖数据层是否有「被丢弃的不可追踪 benefit」与「对不上的返现入账」。查 `fetchAllBenefits` 的过滤逻辑（现状 drop 掉 spend-to-unlock/pass 类）与 trackers 响应；**有数据才做，没数据的行直接不渲染**（不许造假数据），PR 里说明哪个做了哪个没做及原因。
...
## 4. 需要用户拍板的事项（agent 遇到先停）

1. **按类目分组**：若 FINDINGS/fixtures 确认没有类目字段，G2 的「按类目」直接砍掉还是希望另行抓包补数据？（默认：砍掉）
2. **Benefits 特殊行**（无法自动追踪 / 未归类返现）：若现有 trackers 数据不支持，是否接受本轮不做？（默认：不做，行不渲染）
3. 宽模式**默认值**：新用户首次打开默认边栏还是按屏宽自动？（默认：边栏，用户手动 ⤢ 后记住）
