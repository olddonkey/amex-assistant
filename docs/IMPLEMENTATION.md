...
## 10. v0.20.0 — Redeem 追踪（设计稿 4a）

- **Offers 拆双子视图**：搜索框下新增「可加 / 已加」下划线子 Tab（带计数）。「可加」= 原 enroll 列表原样不动；「已加」= 返现追踪视图，右上角「按到期 ▾」。
- **「已加」视图**（`buildAddedIndex` / `addedStats`，纯函数可测）：把所有卡的已加 offer 按分组 key 跨卡聚合，卡行**常开**（同 Benefits 规则）；每张卡标「✓ 已返现 [$X] [· M/D]」或「未见返现」。按到期紧急度升序，**全部返现的组置灰沉底**（含只出现在 savings 列表、已从 added 列表消失的 offer——取并集）。顶部三格统计：已返现总额（绿）/ 待消费组数（藏青）/ 7 天内过期组数（红）。
- **诚实口径**：状态只按**返现入账记录**归类——「未见返现」不是「没消费」；底部脚注注明入账延迟 1–5 天、以 Amex 为准。金额 / 日期字段按候选路径解析（`SAVINGS_LANDING` 响应字段未完全确认），缺失时降级为只显示 ✓。
- **积分与美元分账**（`isPointsRedemption`）：MR 积分 / 里程类 offer 的入账数值是点数不是钱（如 "Earn +1 Membership Rewards point…" 返 5000 点），按积分字段与 offer 文案识别，显示为「5,000 点 / pts」，**永不计入美元合计**；组内「共返」按 `$X + N 点` 分开展示，头部统计只算美元。积分入账同样算「已返现」参与 x/y 计数与置灰。
- **数据获取**：`fetchRedeemedOffers`（`SAVINGS_LANDING`，兼容 page1 / 扁平数组两种形态，走 `retryTransient`）；首次切到「已加」才逐卡懒加载（capped 并发），缓存到下次面板刷新；失败显示重试条，不渲染误导性的全「未见返现」列表。
- 到期天数为日历日差（7/7 看 7/9 = 还剩 2 天），无年份的 `M/D` 过期文本按"未来最近一次"解析。词表新增 15 个 key（双语，parity 测试覆盖）；版本 0.19.2 → 0.20.0；新增 `test/added.test.mjs`（59 个测试全绿）。
