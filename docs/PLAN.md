...
5. **Dry-run 开关**：面板上可勾；勾上时只在进度区打印「将要给卡 X 加 offer Y」，不真正发 enroll。
6. **事后校验**：涉及的卡加完后重新调 `ADDEDTOCARD_LANDING` 读已加列表，用**分组 key** 对比目标是否真的在里面。
7. **结果报告**：每个 offer×卡对归类为「已校验成功 / 请求失败或异常 / 请求成功但校验未出现（假成功）/ 校验读取失败（无法判定）」，分开列出。
8. **低频使用**：README 提示一天跑一两次即可。
...
### M2 — 面板渲染聚合视图（只读，不改账户）
- Shadow DOM 面板：顶部卡范围过滤 + 搜索 + Dry-run 勾选；主体是 offer 列表（名称、可加X卡/已加Y卡徽标、复选框、展开按钮）；展开显示该 offer 的逐卡复选框。
- 一个悬浮开关按钮打开/关闭面板；面板数据来自 M1 的聚合结果。
- 验收：面板正确列出去重后的 offer 与每个 offer 的卡分布；勾选/展开/搜索/卡过滤交互正常；此阶段「加入所选」可先只打印选中的 offer×卡对（等价 dry-run）。
